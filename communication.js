...
function findTrack(trackName,success,fail){
	url = lastFmTrackSearchURL+trackName
	$.get(url, function(response){
		response = parseJson(response);
...
function findArtist(artist,success,fail){
	url = lastFmArtistSearchURL+artist
	$.get(url, function(response){
		response = parseJson(response);
...
function findAlbum(albumName,success,fail){
	url = lastFmAlbumSearchURL+albumName
	$.get(url, function(response){
		response = parseJson(response);
...
function getAlbumInfo(album,success,fail){
	url = lastFmAlbumGetURL +'&artist='+album.artist+'&album='+album.name;
	$.get(url, function(response){
		response = parseJson(response);
...
function findSimilarTracks(track,success,fail){
	url = lastFmSimilarTracksURL+'&artist='+track.artist+'&track='+track.name;
	$.get(url, function(response){
		response = parseJson(response);
...
function findArtistAlbums(artist,f){
	url = lastFmArtistAlbumSearchURL+artist.name
	$.get(url, f);	
}

function findArtistTopTracks(artist,success,fail){
	url = lastFmArtistTopTracksSearchURL+artist.name;
	$.get(url, function(response){
		response = parseJson(response);
...
function findGenreTopTracks(genre,success,fail){
	url = lastFmGenreTopTracksSearchURL+genre.name;
	$.get(url, function(response){
		response = parseJson(response);
		if (response != "" && response.toptracks.track !== undefined)
			success(response.toptracks.track);
